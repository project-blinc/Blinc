//! Surface materials and cursor styling.
//!
//! `Material` selects how an element's surface is shaded: a flat fill, a
//! blurred glass pane, a brushed-metal sheet, or a wood grain. Each variant
//! carries its own parameter struct, so the renderer reads shading inputs
//! straight off the style instead of going through a lookup table.
//!
//! `RenderLayer` sits here too, because which pass an element paints in is
//! decided by whether anything above it is glass.
